...
def create_bill_of_materials(data):
  ...
  title_string = 'Ref,Qty,Footprint,Footprint Library,Symbol,Symbol Library,Datasheet'
  ...
  with open(outfile,'w') as obom:
    ...
    for b in bom:
      ...
      obom.write(b.refs+','+str(b.qty)+','+b.footprint+','+b.footprint_lib+','+ \
  ...
  with open(outfile,'w') as obom:
    obom.write('Ref,Qty,Description,MF,MF_PN,S1,S1_PN,Type\n')
    for b in bom:
      bf_desc = ''
      bf_mf_name = ''
      bf_mf_pn = ''
